		...
		},
		{
			...
			Cell: ({ row: { original } }: { row: { original: Certificate } }) => {
				...
				const removeFavoriteFromList = () =>
					favoriteList?.filter((cert) => cert.uniqueNumber !== favoriteCertificate?.uniqueNumber);
				const addFavoriteToList = () => [...((favoriteList as Certificate[]) ?? []), original];

				const addOrRemoveFavorite = () =>
					setFavorite(() => (favoriteCertificate ? removeFavoriteFromList() : addFavoriteToList()));
